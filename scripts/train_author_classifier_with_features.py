...
def build_combined_feature_names(word_vectorizer, char_vectorizer) -> list[str]:
    feature_names = []
...
def get_top_features_per_user(model, feature_names: list[str], top_n: int = 20) -> dict[str, list[str]]:
    output = {}

    for i, label in enumerate(model.classes_):
        coef = model.coef_[i]
        top_indices = coef.argsort()[-top_n:][::-1]
        output[label] = [feature_names[idx] for idx in top_indices]

    return output


def save_top_features(top_features: dict[str, list[str]], output_path: Path) -> None:
    with output_path.open("w", encoding="utf-8") as f:
        for label, features in top_features.items():
            f.write(f"Top features for {label}:\n")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="How many top features to save per user",
    )
    parser.add_argument(
        "--save-top-features",
        action="store_true",
        help="Save top features per user to a local txt file",
    )
    ...
    # features
    feature_names = build_combined_feature_names(word_vectorizer, char_vectorizer)
    top_features = get_top_features_per_user(model, feature_names, top_n=args.top_n)

    print("\n=== TOP FEATURES PER USER ===")
    for label, features in top_features.items():
        print(f"\nTop features for {label}:")
        for feat in features:
            print(f"  {feat}")

    if args.save_top_features:
        output_path = Path(f"top_features_{args.features}.txt")
        save_top_features(top_features, output_path)
        print(f"\nSaved top features to {output_path}")
